...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        duplicates = find_duplicates(root, args.min_tokens, args.filters, args.max_groups, args.include_tests, args.max_occurrences_per_window)
    except (OSError, ValueError, DryError) as error:
        print(f"dry4swift: {error}", file=sys.stderr)
        return 1
    payload = {"schema_version": 1, "tool": "dry4swift", "version": __version__, "root": root.as_posix(), "summary": {"groups": len(duplicates), "min_tokens": args.min_tokens}, "duplicates": [duplicate.to_dict() for duplicate in duplicates]}
    if args.json_output:
